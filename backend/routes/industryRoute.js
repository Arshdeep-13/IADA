...
const finalMaintenanceBill = require("../models/finalMaintenanceBill");
...
const { sendOtpOnPhone } = require("../controllers/phoneOtpController");
...
router.post("/zoned-with-meter", authMiddleware, async (req, res) => {
  try {
    ...
    const industries = await Industry.find({
      ...
      "-password -__v -is_registered -is_approved -_id -gstin_number -currentToken -item_manufactured -no_of_employees -no_of_employees_HIM -lessee -industry_area -industry_email -phone_number -zone_id"
    );
...
router.post("/login", async (req, res) => {
  try {
    ...
    const secretKey = process.env.PASS_SECRET; // Use the same secret key
    ...
    if (data.success) {
      ...
      // let data;
      let data = { statusCode: "SC0000" };
      try {
        // const result = await sendOtpOnPhone(
        //   industry.phone_number,
        //   industry.email
        // );
        // data = await result.json();
        if (data.statusCode === "SC0000") {
          res.status(200).send({
            status: 200,
            email: email,
...
router.get("/isBillDue", async (req, res) => {
  try {
    const authHeader = req.headers.authorization;
    const token = authHeader.split(" ")[1];
    const decoded = jwt.verify(token, process.env.SECRET_KEY);
    ...
    const finalwaterbill = await finalWaterBill.findOne({
      premises: plot_number,
    });
    const final_maintenance_bill = await finalMaintenanceBill.findOne({
      premises: plot_number,
    });

    if (finalwaterbill || final_maintenance_bill) {
      if (!finalwaterbill) {
        return res.status(200).json({
          isWaterBillDue: "false",
          isMaintenanceBillDue: `${final_maintenance_bill.isDue}`,
        });
      } else if (!final_maintenance_bill) {
        return res.status(200).json({
          isWaterBillDue: `${finalwaterbill.isDue}`,
          isMaintenanceBillDue: "false",
        });
      } else {
        return res.status(200).json({
          isWaterBillDue: `${finalwaterbill.isDue}`,
      ...
      return res.status(404).json({ message: "No bill found", success: false });
    }
  } catch (err) {
