...
const enterBill = async (industry, industryBill, zoneId) => {
  try {
    ...
    // The bill which is presented after calculation is the 'presentedBill' defined below.
    ...
    dueBills.forEach((bill) => {
      billWithLateFine += //late fine on prev bills
        ...
        parseFloat(bill.amountPayOnDueDate); //applied seperately on each bill
    ...
    industryBill.amountPayAfterDueDate = billWithLateFine;
    ...
    if (calculatedFinalBill === 0) {
      ...
      calculatedArrears = Math.abs(
        calculatedFinalBill - (newBill + industryBill.sewerageCharges)
      );
      // Calculate arrears as the sum amount of previous bills
    }
    // Save the new bill
    ...
    const updateData = {
      ...
      amountPayOnDueDate: newBill + calculatedArrears, //this is only the current readings cost+sewerage+arrears {basically the complete present bill}
      latePaymentSurcharge: surchargeOfFinal,
      amountPayAfterDueDate: afterDueAmt,
      currentTotal: calculatedFinalBill,
